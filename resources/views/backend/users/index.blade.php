        ...
        <table class="table table-hover">
            ...
            @forelse($users as $user)
                <tr>
                    <td>
                        @if ($user->user_image != '')
                            <img class="rounded-circle" src="{{ asset('assets/users/' . $user->user_image) }}" width="60" height="60" alt="{{ $user->full_name }}">
                        @else
                            <img src="{{ asset('assets/users/avatar.png') }}" width="60">
                        @endif
                    </td>
                    <td>
                        <a href="{{ route('admin.users.show', $user->id) }}">{{ $user->full_name }}</a>
