...
@section('title', 'Edit ShippingCompany')
...
@section('content')
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            <h6 class="m-0 font-weight-bold text-primary">Edit ShippingCompany {{ $shippingCompany->name }}</h6>
            <div class="ml-auto">
        ...
        <div class="card-body">
            ...
            {{-- {{ dd(route('admin.shipping_companies.update', $shippingCompany->id)) }} --}}
            ...
            <form action="{{ route('admin.shipping_companies.update', $shippingCompany->id) }}" method="post">
                ...
                <div class="row pt-4">
                    <div class="col-3">
                        ...
                        <select name="fast" class="form-control">
                            <option value="1" {{ old('fast', $shippingCompany->fast) == 1 ? 'selected' : null }}>Fast Delivery</option>
                            <option value="0" {{ old('fast', $shippingCompany->fast) == 0 ? 'selected' : null }}>Normal Delivery</option>
                        </select>
                    ...
                    <div class="col-12">
                        ...
                        <select name="countries[]" class="form-control select2" multiple="multiple">
                            @forelse ($countries as $country )
                                <option value="{{ $country->id }}" {{ in_array($country->id, old('countries', $shippingCompany->countries->pluck('id')->toArray() )) ? 'selected' : null }}>{{ $country->name }}</option>
                            @empty
                            @endforelse
                        </select>
                ...
                <div class="form-group pt-4">
                    <button type="submit" name="submit" class="btn btn-primary">Update ShippingCompany</button>
                </div>
            </form>
        </div>
    </div>
